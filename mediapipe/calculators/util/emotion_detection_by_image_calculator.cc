...
absl::Status EmotionDetectionByImageCalculator::Process(CalculatorContext* cc) 
{
  ...
  if (has_detection_from_vector) {
    for (const auto& detection : cc->Inputs().Tag(kDetectionsTag)
      ...
      //! 截取面部区域
      cv::Mat faceMat = CVMat(cv::Rect(leftTopX, leftTopY, 
      ...
      // std::vector<float> input_data(fixed_size_face.begin<float>(), fixed_size_face.end<float>());
      // 扩展维度
      std::vector<cv::Mat> input_data = { fixed_size_face };
